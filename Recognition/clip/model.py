...
from modules.stdha import STDHA
...
class ShiftAttnBlock(nn.Module):
    def __init__(self,
        ...
        self.attn = STDHA(dim, dim//64, T, shift_div=dim//64, kernel_size=kernel_size, learnable_shift=False)
    ...
    def forward(self, x, x_token=None, side_spatial_pos_embed=None, side_temporal_pos_embed=None, use_ckpt=False):
        n, bt, d = x.size()
        h = int(x.shape[0] ** 0.5)
        # temporal conv
        ...
        xt = rearrange(xt, 'n (b t) d -> n b t d', t=self.T)
        xt[1:] = xt[1:] + side_spatial_pos_embed.unsqueeze(1).unsqueeze(1)
        xt = xt + side_temporal_pos_embed.unsqueeze(0).unsqueeze(0)
        xt = rearrange(xt, 'n b t d -> n (b t) d')
        if use_ckpt:
...
class SideNetwork(nn.Module):
    def __init__(self,
        ...
        for i in range(len(self.side_layers)):
            self.resblocks.append(ShiftAttnBlock(self.side_dim, int(self.side_dim * self.temporal_ratio), kernel_size=1, T=self.T))
        ...
        self.side_temporal_position_embeddings = nn.Parameter(side_scale * torch.randn((T, self.side_dim)))
        # SELFY block
        ...
        nn.init.normal_(self.side_temporal_position_embeddings, std=0.01)
    ...
    def forward(self, x: torch.tensor, x_img: torch.tensor):
        ...
        for i_vid, i_img in enumerate(self.side_layers):
            ...
            x = self.resblocks[i_vid](x, x_token,
                                      ...
                                      self.side_temporal_position_embeddings)
        return x
